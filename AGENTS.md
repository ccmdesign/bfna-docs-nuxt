...
`contentImporter.js` relies on Contentful and YouTube credentials. Ensure `.env` contains `CONTENTFUL_SPACE_ID`, `CONTENTFUL_ACCESS_TOKEN`, and `YOUTUBE_API_KEY` before running dev or generate commands. The Directus importer (`directus/films.js`) additionally uses `DIRECTUS_TOKEN` to cache resolved animated-thumbnail URLs back into `docs_documentaries.animated_thumbnail`; missing the var produces a single non-fatal warning and skips the writeback. See `.env.example` for the full set. When offline you may commit temporary JSON in `content/`, but drop stubs before merging so automated imports remain the source of truth.
